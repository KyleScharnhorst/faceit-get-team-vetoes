...
def update_map_play_data(map, picked, banned, random_ban, win, unfinished):
                    ...
                    for entity in ticket.entities:
                        ...
                        if entity.selected_by == faction:
                            if entity.status == BAN_STATUS:
                                if entity.random:
                                    update_map_play_data(entity.guid, 0, 0, 1, 0, 0)
                                else:
                                    update_map_play_data(entity.guid, 0, 1, 0, 0, 0) 
                            elif entity.status == PICK_STATUS:
                                if is_match_finished(match):
                                    if is_map_won(match):
                                        update_map_play_data(entity.guid, 1, 0, 0, 1, 0)
                                    else:
                                        update_map_play_data(entity.guid, 1, 0, 0, 0, 0)
                                else:
                                    update_map_play_data(entity.guid, 1, 0, 0, 0, 1)
                            else:
